...
function PatientMessageList({
  ...
  const bubbleMine = role === 'patient' ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-900';
  const bubbleOther = role === 'patient' ? 'bg-gray-100 text-gray-900' : 'bg-cyan-600 text-white';
  const metaMine = role === 'patient' ? 'text-cyan-100' : 'text-gray-500';
  const metaOther = role === 'patient' ? 'text-gray-500' : 'text-cyan-100';
      ...
      {messages.map((m) => {
        ...
        return (
          <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[88%] rounded-2xl px-3.5 py-2.5 text-sm shadow-sm ${mine ? bubbleMine : bubbleOther}`}
            >
              <p className="whitespace-pre-wrap break-words">{m.body}</p>
              <div
                className={`mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] ${
                  mine ? metaMine : metaOther
                }`}
              >
                <span>
...
function PatientScrollMessages({
  isLoading,
  messages,
  role,
  bottomRef,
  ...
  return (
    <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
      {isLoading ? (
        ...
        <p className="py-6 text-center text-sm text-gray-500">No messages yet.</p>
      ) : (
        <PatientMessageList messages={messages} role={role} />
      )}
      <div ref={bottomRef} />
    </div>
  );
...
function PatientChatComposer({ text, sendPending, onChange, onSubmit, onKeyDown }: ComposerProps) {
  return (
    <form onSubmit={onSubmit} className="shrink-0 border-t border-gray-100 bg-gray-50/50 p-3">
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          className="btn-primary flex shrink-0 items-center gap-1 self-end bg-cyan-600 px-4 py-2.5 hover:bg-cyan-700"
...
/**
 * Message thread with cyan accent for the patient side.
 */
export function ChatThread({
  ...
  return (
    <div className="flex h-[min(72vh,580px)] min-h-[300px] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      {peerName ? (
        <header className="shrink-0 border-b border-gray-100 bg-gradient-to-r from-cyan-50/90 to-white px-4 py-3">
