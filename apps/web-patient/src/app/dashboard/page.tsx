    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="glass sticky top-0 z-40 border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { clearAuth(); router.push('/'); }} className="btn-ghost p-2 text-gray-500 hover:text-red-600">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-8 animate-in">
          ...
          <p className="text-gray-500 mt-1">What do you need today?</p>
        </div>
                ...
                return (
                  <div key={appt.id} className="card p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="font-medium text-gray-800 text-sm">Dr. {u.firstName} {u.lastName}</div>
                      <div className="text-gray-500 text-xs">{doc?.specialization}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-gray-700 text-sm">{formatDate(appt.appointmentDate)}</div>
                      <div className="text-gray-500 text-xs">{formatTime(appt.startTime)}</div>
                    </div>
        ...
        {past.length > 0 && (
          <div>
            <h2 className="font-semibold text-gray-800 mb-4">Recent Visits</h2>
            <div className="space-y-3">
              {past.slice(0, 3).map((appt: any) => {
                ...
                return (
                  <div key={appt.id} className="card p-4 flex items-center gap-4 opacity-60">
                    <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center shrink-0 font-bold text-gray-500">
                      {u.firstName?.[0]}{u.lastName?.[0]}
                    </div>
                    <div className="flex-1">
                      <div className="font-medium text-gray-700 text-sm">Dr. {u.firstName} {u.lastName}</div>
                      <div className="text-gray-400 text-xs">{formatDate(appt.appointmentDate)}</div>
                    </div>
                    <span className={getStatusColor(appt.status)}>{appt.status}</span>
                  </div>
                );
