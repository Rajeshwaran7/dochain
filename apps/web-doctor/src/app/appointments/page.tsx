  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="glass sticky top-0 z-30 border-b border-gray-200 px-6 h-14 flex items-center gap-4">
        ...
        <h1 className="font-semibold text-gray-800">Appointments</h1>
      </header>
          ...
          {TABS.map(tab => (
            <button key={tab.value} onClick={() => setActiveTab(tab.value)}
              className={`flex-1 py-2 text-sm rounded-lg font-medium transition-all ${
                activeTab === tab.value ? 'bg-violet-600 text-white' : 'text-gray-500 hover:text-gray-700'
              }`}>
              {tab.label}
            </button>
          ))}
              ...
              return (
                <div key={appt.id} className="card overflow-hidden">
                  ...
                  >
                    ...
                    {appt.symptoms && (
                      <p className="text-gray-400 text-xs mt-3 pl-14">Symptoms: {appt.symptoms}</p>
                    )}
                  </div>
