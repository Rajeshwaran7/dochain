...
import {
  Calendar, Users, Star, CreditCard, Settings, Menu,
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {[
            ...
            <Link key={item.href} href={item.href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all text-sm"
            >
        ...
        <div className="px-3 pb-4 border-t border-gray-200 pt-4">
          <div className="flex items-center gap-3 px-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-900 text-sm font-medium truncate">Dr. {user.firstName}</div>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 bg-white border-r border-gray-200 flex flex-col animate-in">
            <div className="px-5 py-5 border-b border-gray-200 flex items-center justify-between">
              <div>
                <Link href="/dashboard" className="font-bold text-violet-600 text-lg">Dochain</Link>
                <div className="text-gray-600 text-xs mt-0.5">Doctor Portal</div>
              </div>
              <button onClick={() => setSidebarOpen(false)} className="text-gray-500 hover:text-gray-900 p-1">✕</button>
            </div>
            <nav className="flex-1 px-3 py-4 space-y-1">
              {[
                { icon: <TrendingUp className="w-4 h-4"/>, label: 'Dashboard', href: '/dashboard' },
                ...
                <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-700 hover:text-gray-900 hover:bg-gray-100 transition-all text-sm">
                  {item.icon} {item.label}
                </Link>
              ))}
            </nav>
      ...
      <div className="md:pl-56">
        ...
        <main className="p-6 space-y-6">
          ...
          <div className="animate-in">
            ...
            <p className="text-gray-600 text-sm mt-1">Here's what's happening with your practice today.</p>
          </div>
                  ...
                  {todayAppts.slice(0, 5).map((appt: any) => {
                    ...
                    return (
                      <div key={appt.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-gray-900 text-sm truncate">{p.firstName} {p.lastName}</div>
                          <div className="text-gray-600 text-xs">{appt.startTime}</div>
                        </div>
            ...
            <div className="card p-5 border-amber-500/30 bg-amber-500/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-gray-600 text-sm">Your doctor profile is being verified. This usually takes 24-48 hours.</div>
                </div>
              </div>
            </div>
          )}
